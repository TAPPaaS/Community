#!/usr/bin/env bash
#
# shelly-fleet:control install-service — no-op (policy-only module).
#
# The 'control' service is purely declarative: it exposes its ports via
...
info "shelly-fleet:control install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole handled by consumer's firewall:rules)."
