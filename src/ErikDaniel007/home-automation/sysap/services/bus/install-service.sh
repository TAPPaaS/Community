#!/usr/bin/env bash
#
# sysap:bus install-service — no-op (policy-only module).
#
# The 'bus' service is purely declarative: it exposes its ports via
...
info "sysap:bus install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole handled by consumer's firewall:rules)."
