#!/usr/bin/env bash
#
# reolink:rtsp install-service — no-op (policy-only module).
#
...
info "reolink:rtsp install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole handled by consumer's firewall:rules)."
