#!/usr/bin/env bash
#
# reolink:api install-service — no-op (policy-only module).
#
...
info "reolink:api install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole handled by consumer's firewall:rules)."
