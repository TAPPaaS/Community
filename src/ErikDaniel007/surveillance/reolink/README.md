...
Reolink surveillance cameras on the household camera network (`iotCams`).
...
|---|---|---|
| Live RTSP stream | Home Assistant (`srvHome`) | HA generic camera or native Reolink integration |
| Recording + alerts | Home Assistant (`srvHome`) | HA Reolink integration |
...
- Reolink camera(s) on the `iotCams` network (VLAN 430)
...
| `firewall:discovery` | mDNS relay (`iotCams` ↔ `srvHome`/`home`) so ONVIF/mDNS camera discovery works across VLANs (added in v0.2.0) |
