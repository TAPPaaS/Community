#!/usr/bin/env bash
#
# alfen:discovery install-service — no-op (policy-only module).
#
# The 'discovery' service is purely declarative: it exposes its ports via
...
info "alfen:discovery install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole handled by consumer's firewall:rules)."
