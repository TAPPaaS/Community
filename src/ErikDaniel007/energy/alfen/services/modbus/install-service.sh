#!/usr/bin/env bash
#
# alfen:modbus install-service — no-op (policy-only module).
#
...
info "alfen:modbus install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole handled by consumer's firewall:rules)."
