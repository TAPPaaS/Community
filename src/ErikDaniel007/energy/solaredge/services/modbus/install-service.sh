#!/usr/bin/env bash
#
# solaredge:modbus install-service — no-op (policy-only module).
#
# The 'modbus' service is purely declarative: it exposes its ports via
...
info "solaredge:modbus install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole handled by consumer's firewall:rules)."
