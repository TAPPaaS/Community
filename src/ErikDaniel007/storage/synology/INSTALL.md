...
2. **Static DHCP reservation** — assign a fixed IP to the Synology MAC address (bond MAC if LAGG is used) on the `srv` network (VLAN 210) via OPNsense: Services → DHCPv4 → VLAN210 → Static Mappings → Add.
3. **DNS host override** — add via `dns-manager`:
   ```bash
   dns-manager --no-ssl-verify add synology srv.internal <ip>
   ```
4. **DSM reachable** at `https://synology.srv.internal:5001` before applying this module.
...
|---|---|---|---|---|
| DSM | 5000 | 5001 | `synology.srv.internal` | — |
| Synology Drive | 10002 | 10003 | `drive.srv.internal` | — |
| Synology Photos | 5080 | 5443 | `photos.srv.internal` | — |
| Surveillance Station | 9900 | 9901 | `surveillance.srv.internal` | — |
| Active Backup M365 | 28003 | 28004 | `m365backup.srv.internal` | — |
| Active Backup Business | 8001 | 8002 | `activebusiness.srv.internal` | — |
| File Station | — | — | — | — |

Then add per-service DNS overrides (all point to the same Synology IP):

```bash
dns-manager --no-ssl-verify add drive           srv.internal <ip>
dns-manager --no-ssl-verify add photos          srv.internal <ip>
dns-manager --no-ssl-verify add surveillance    srv.internal <ip>
dns-manager --no-ssl-verify add m365backup      srv.internal <ip>
dns-manager --no-ssl-verify add activebusiness  srv.internal <ip>
```
...
1. In Authentik: create OAuth2/OIDC provider. Redirect URI:
   `https://synology.srv.internal:5001/webman/sso/SSOOauth.cgi`
...
| Check | Expected |
|---|---|
| `https://synology.srv.internal:5001` | DSM login page |
| `https://drive.srv.internal:10003` | Synology Drive portal |
| `https://photos.srv.internal:5443` | Synology Photos portal |
| `https://surveillance.srv.internal:9901` | Surveillance Station |
| SMB mount from home client | Share visible and writable |
| NFS mount from TAPPaaS VM | `mount synology.srv.internal:/volume1/share /mnt/test` succeeds |
