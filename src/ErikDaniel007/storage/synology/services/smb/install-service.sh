#!/usr/bin/env bash
#
# synology:smb install-service — no-op (policy-only module).
#
# The 'smb' service is purely declarative: it exposes its ports via
...
info "synology:smb install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole handled by consumer's firewall:rules)."
