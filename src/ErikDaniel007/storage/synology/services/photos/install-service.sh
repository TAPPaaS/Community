#!/usr/bin/env bash
#
# synology:photo install-service — no-op (policy-only module).
#
# The 'photo' service is purely declarative: it exposes its ports via
...
info "synology:photo install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole handled by consumer's firewall:rules)."
