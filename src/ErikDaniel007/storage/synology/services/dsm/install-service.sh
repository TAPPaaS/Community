#!/usr/bin/env bash
#
# synology:dsm install-service — no-op (policy-only module).
#
# The 'dsm' service is purely declarative: it exposes its ports via
...
info "synology:dsm install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole handled by consumer's firewall:rules)."
