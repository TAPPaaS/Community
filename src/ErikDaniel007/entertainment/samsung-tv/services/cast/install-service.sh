#!/usr/bin/env bash
#
# samsung-tv:cast install-service — no-op (policy-only module).
#
# The 'cast' service is purely declarative: it exposes its ports via
...
info "samsung-tv:cast install-service for consumer '${CONSUMER}' — no provider-side work needed (auto-pinhole handled by consumer's firewall:rules)."
