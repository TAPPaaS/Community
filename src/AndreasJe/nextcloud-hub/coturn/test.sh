...
subheader "Timer: coturn-backup.timer"
BACKUP_TIMER=$(remote "systemctl is-active coturn-backup.timer 2>/dev/null" || echo "unknown")
if [ "$BACKUP_TIMER" = "active" ]; then
    pass "coturn-backup.timer is active"
elif [ "$BACKUP_TIMER" = "unknown" ] || [ "$BACKUP_TIMER" = "inactive" ]; then
    skip "coturn-backup.timer is ${BACKUP_TIMER} — not yet configured or not yet started"
else
    fail "coturn-backup.timer is ${BACKUP_TIMER} (expected active)"
fi
