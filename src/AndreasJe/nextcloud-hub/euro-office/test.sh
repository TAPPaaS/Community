...
EXAMPLE_ENABLED_CFG=$(python3 -c "import json; print(json.load(open('euro-office.json')).get('exampleEnabled','false'))" 2>/dev/null || echo "false")

if [ "$CONTAINER_OK" = false ]; then
    skip "Example app check skipped — container is not running"
elif [ "$EXAMPLE_ENABLED_CFG" != "true" ]; then
    skip "Example app check skipped — exampleEnabled=false in euro-office.json"
else
...
# Test 6: Admin Panel
# ============================================================================
# URL varies by image version: /admin/ (nightly) or /adminpanel/ (stable).
...
if [ "$CONTAINER_OK" = false ]; then
    ...
    if [ "$ADMIN_CODE" = "200" ] || [ "$ADMIN_CODE" = "301" ] || [ "$ADMIN_CODE" = "302" ]; then
        ...
        skip "Admin panel backend not available in this image build (/admin/: ${ADMIN_CODE}, /adminpanel/: ${ADMIN_CODE2}) — expected on nightly builds where the admin panel service may not be active"
    else
