...
# on-host; only the HOME (OPNsense) *public* key is referenced.
{
  ...
  roles = [ ]; # minimal first deploy: tunnel + base only. Add "reverse-proxy"/"admin-vpn"/"backup"/"smtp-relay" later.
  ...
  # smtp-relay role only (opt-in — see roles above). hostname empty = role
  # inert even if listed in roles, since the ACME cert/Postfix hostname has
  # nowhere to point yet. satellite-manager fills this in from
  # satellite-<name>.json's smtpRelay.hostname when the role is requested.
  smtpRelay = {
    hostname = "";
