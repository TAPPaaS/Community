...
main() {
    ...
    scp ${SSH_OPTS} "${SCRIPT_DIR}/wordpress.nix" "tappaas@${VM_HOST}:/tmp/wordpress.nix"
    # shellcheck disable=SC2086
    ssh ${SSH_OPTS} "tappaas@${VM_HOST}" "sudo mv /tmp/wordpress.nix /etc/nixos/wordpress.nix && sudo nixos-rebuild switch"
