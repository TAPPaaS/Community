...
#
# Network: srv zone (VMID 312, tappaas2), firewall ports 22 (SSH) + 80 (HTTP)
...
let
  ...
  versions = {
    documentServerImage = "ghcr.io/euro-office/documentserver:latest";
  };
in
{
  # ============================================================================
...
EOF
  ...
  # and patch local.json after start to disable JWT (built-in example app doesn't sign tokens)
  systemd.services."podman-euro-office" = {
    requires = [ "euro-office-init-secrets.service" ];
    after    = [ "euro-office-init-secrets.service" ];
    # Prefix with - so ExecStartPost failure doesn't kill the container service
    serviceConfig.ExecStartPost = "-${pkgs.writeShellScript "euro-office-disable-jwt" ''
      # Wait for supervisor socket to appear inside the container (up to 120s)
      for i in $(seq 1 24); do
        if ${pkgs.podman}/bin/podman exec euro-office test -S /var/run/supervisor.sock 2>/dev/null; then
          break
        fi
        sleep 5
      done

      # Patch local.json: disable JWT token verification
      ${pkgs.podman}/bin/podman exec euro-office sh -c '
      ...
      # Restart docservice and converter to pick up the config change
      ${pkgs.podman}/bin/podman exec euro-office supervisorctl restart docservice converter || true
    ''}";
  };
